...
{
	case "photo":
		$rec_obj = FDB::fetchFirst('SELECT * FROM '.FDB::table('share_photo').' WHERE id = '.$id);
		if($rec_obj['base_id'] > 0)
		{
			$base_obj = FDB::fetchFirst('SELECT * FROM '.FDB::table('share_photo').' WHERE id = '.$rec_obj['base_id']);
			if($base_obj)
				$rec_obj = $base_obj;
		}
	break;
	...
	case "goods":
		$rec_obj = FDB::fetchFirst('SELECT * FROM '.FDB::table('share_goods').' WHERE id = '.$id);
		if($rec_obj['base_id'] > 0)
		{
			$base_obj = FDB::fetchFirst('SELECT * FROM '.FDB::table('share_goods').' WHERE id = '.$rec_obj['base_id']);
			if($base_obj)
				$rec_obj = $base_obj;
		}
	break;
...
$img = FS('Image')->formatById($rec_obj['img_id']);
$rec_img = $img['img'];
$rec_obj['type'] = $type;
