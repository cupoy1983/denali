...
$cache_key = 'ubao/'.substr($cache_key,0,2).'/'.substr($cache_key,2,2).'/'.$cache_key;
...
if($html == NULL)
{
	...
	$sql = 'SELECT DISTINCT(share_id) FROM '.FDB::table("share_user_goods").' WHERE uid = '.$uid.' ORDER BY share_id DESC';
