 ...
 * 会员试用管理
 */
class TrialOrderAction extends CommonAction{
    public function index(){
		...
		$sql = 'SELECT COUNT(id) AS tcount
			FROM '.C("DB_PREFIX").'trial_order '.$where;

		$count = $model->query($sql);
		$count = $count[0]['tcount'];

		$sql = 'SELECT * FROM '.C("DB_PREFIX").'trial_order '.$where;
	...
	public function show()
	{
		...
		$order = D("TrialOrder")->where("id = $id")->find();
