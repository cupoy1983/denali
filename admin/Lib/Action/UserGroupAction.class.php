...
class UserGroupAction extends CommonAction
{
	...
	{
		$model = M();
		$sql = 'SELECT COUNT(gid) AS tcount FROM '.C("DB_PREFIX").'user_group';
		$count = $model->query($sql);
		$count = $count[0]['tcount'];

		$sql = 'SELECT *,IF(is_special = 1,\'\',CONCAT(credits_lower,\' ～ \',credits_higher)) AS credits_range FROM '.C("DB_PREFIX").'user_group';
		$this->_sqlList($model,$sql,$count,array(),'gid',true);
		$this->display();
	}
	...
	{
		$_POST['is_special'] = (int)$_POST['is_special'];
	...
	public function edit()
	{
		...
		$uga_list = array();
		$ug_authoritys = D('UserGroupAuthority')->where('gid = '.$id)->select();
	...
	public function update()
	{
		...
		if (false !== $list)
		{
			...
			if($_POST['reset_group_access'] == 1)
			{
				$users = D('User')->where('gid = '.$id)->field('uid')->select();
				foreach($users as $user)
				{
					D('UserAuthority')->where('uid = '.$user['uid'])->delete();
					foreach($access_list as $module => $actions)
					{
						$index = 0;
						foreach($actions as $action)
