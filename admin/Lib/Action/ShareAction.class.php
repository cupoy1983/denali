...
class ShareAction extends CommonAction
{
	...
	function edit()
	{
		...
		$share_colors = array();
		$res = FDB::query("select color_id from ".FDB::table("share_color")."  where share_id = ".$share['share_id']);
		while($data = FDB::fetch($res))
		{
			$share_colors[$data['color_id']] = 1;
		}
		$this->assign ('share_colors',$share_colors );
	...
	public function update() {
		...
		if (false !== $list) {
			...
			FS('Share')->updateShareColor($data['share_id'],explode(",",$_REQUEST['colors']));
