...
class AdvAction extends CommonAction
{
	public function index()
	{
		...
		$ap_list = M("AdvPosition")->where('status = 1')->select();
	...
	public function add()
	{
		...
		{
			$pid = (int)D("AdvPosition")->where("code = '".$_REQUEST['pcode']."'")->getField('id');
			$this->assign("pid",$pid);
			$this->assign("pcode",$_REQUEST['pcode']);
			$this->assign("pcateid",$_REQUEST['pcateid']);
		}
		
		if(isset($_REQUEST['target_key']))
		{
			$this->assign("target_key",$_REQUEST['target_key']);
		}
	...
	public function insert()
	{
		...
		if ($id !== false)
		{
			$upload_list = $this->uploadImages(0,'images',false,array(),true);
			if($upload_list)
			{
