...
class StyleCategoryAction extends CommonAction
{
	...
	public function add()
	{
		$cate_list = D("StyleCategory")->where('status = 1 AND parent_id = 0')->field('cate_id,parent_id,cate_name')->order('sort ASC,cate_id ASC')->select();
	...
	public function edit()
	{
		...
		$childs[] = $id;
		$cate_list = D("StyleCategory")->where('status = 1 AND parent_id = 0 AND cate_id not in ('.implode(',', $childs).')')->field('cate_id,parent_id,cate_name')->order('sort ASC,cate_id ASC')->select();
		$cate_list = D("StyleCategory")->toFormatTree($cate_list,array('cate_name'),'cate_id');
