...
class RegionAction extends CommonAction
{
	...
	public function add()
	{
		$list = D("Region")->where('parent_id = 0')->field('id,parent_id,name')->select();
	...
	public function edit()
	{
		...
		$list = D("Region")->where('parent_id = 0 AND id <> '.$id)->field('id,parent_id,name')->select();
		$list = D("Region")->toFormatTree($list,array('name'),'id');
		$this->assign("list",$list);
		$this->display();
	}
	...
	public function getCitys()
	{
		...
		$city_list = D("Region")->where('parent_id = '.$pid)->select();
