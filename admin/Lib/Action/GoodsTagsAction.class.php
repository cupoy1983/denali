...
class GoodsTagsAction extends CommonAction
{
	...
	{
		$_POST['tag_code'] = $_REQUEST['tag_name'];
		parent::insert();
	}
	...
	public function search()
	{
		...
		$list = array();
		if(empty($where))
			$list = D('GoodsTags')->limit('0,60')->order('sort ASC,tag_id ASC')->select();
		else
			$list = D('GoodsTags')->where('1'.$where)->limit('0,60')->order('sort ASC,tag_id ASC')->select();
