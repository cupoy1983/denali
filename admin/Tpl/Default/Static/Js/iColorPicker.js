...
var imageUrl=STATIC + '/Images/color.png';
function iColorShow(id, id2) {
    var eICP = jQuery("#" + id2).offset();
    jQuery("#iColorPicker").css({
        'top': eICP.top + (jQuery("#" + id2).height()) + "px",
        'left': eICP.left + (jQuery("#" + id2).width()) + "px",
		'width': '238px',
        'position': 'absolute'
    }).fadeIn("fast");
        ...
        for (j = 0; j < tblChilds.length; j++) {
            ...
            for (k = 0; k < tblCells.length; k++) {
                jQuery(tblChilds[j].childNodes[k]).unbind().mouseover(function(a) {
					...
					var rel = jQuery("#" + id).get(0).getAttribute("rel");
					if(rel)
						jQuery("#" + rel).css("color", aaa);
...
this.iColorPicker = function() {
	if(jQuery("#iColorPicker").length == 0)
	{
