...
class MessageService
{
	...
	public function sysMsgInit($uid,$gid)
	{
		...
		if($count > 0)
		{
			$result = FDB::query("INSERT INTO ".FDB::table('user_notice')."(uid, type, num, create_time) VALUES('$uid',7,'$count','".TIME_UTC."')", 'SILENT');
			if(!$result)
				FDB::query("UPDATE ".FDB::table('user_notice')." SET num = num + '$count', create_time='".TIME_UTC."' WHERE uid='$uid' AND type=7");
		}
	}
	
	public function getSysMsgCount($uid)
	{
		$uid = (int)$uid;
		return FDB::resultFirst('SELECT COUNT(*) FROM '.FDB::table('sys_msg_member')." WHERE uid='$uid' AND status < 2");
	}
	 ...
	 * @return void
	 */
	public function getSysMsgs($uid,$limit = '')
	{
		$uid = (int)$uid;
		if(!empty($limit))
			$limit = ' LIMIT '.$limit;
		...
		if(count($mids) > 0)
		{
			FDB::query("UPDATE ".FDB::table('sys_msg_member')." SET status = 1 WHERE uid='$uid' AND status = 0 AND mid IN (".implode(',', $mids).")");
		}
		return $list;
	}
	...
	public function getMsgList($uid,$limit = '')
	{
		...
		$query = FDB::query('SELECT * 
			...
			WHERE mm.uid='$uid' ORDER BY mm.last_dateline DESC $limit");
	...
	public function getMsgsByMlid($mlid,$uid,$limit = '')
	{
		...
		$query = FDB::query('SELECT * 
			...
			WHERE mlid='$mlid' $where ORDER BY dateline DESC $limit");
